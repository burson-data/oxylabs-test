...
def scrape_google_with_oxylabs(query, geo_location):
    ...
    iterating through all available pages until no more results are found.
    ...
    Returns:
        A Pandas DataFrame containing the combined search results, or None on error.
    """
    try:
    ...
    all_news = []  # Initialize an empty list to store all news items
    page_num = 1  # Start with page 1
    while True:  # Loop indefinitely until a break condition is met
        payload = {
            'source': 'google_search',
            'query': query,
            'pages': str(page_num),  # Use the current page number
            'parse': True,
            ...
            try:
                for item in data['results']:
                    ...
                    if 'results' in page_data and 'main' in page_data['results']:
                        news_items = page_data['results']['main']
                        if not news_items:  # Check if there are no results on this page
                            st.info(f"No more results found on page {page_num}. Stopping.")
                            break  # Exit the inner loop if no more results
                        ...
                        st.warning(f"No 'results' or 'main' found on page {page_num}.")
                        break  # Exit the inner loop if the structure is unexpected
    
            except (KeyError, TypeError) as e:
                st.error(f"Error parsing Oxylabs API response on page {page_num}: {e}")
                break  # Exit the outer loop on parsing errors
    
        except requests.exceptions.RequestException as e:
            st.error(f"Error making Oxylabs API request on page {page_num}: {e}")
            break  # Exit the outer loop on request errors
        except Exception as e:
            st.error(f"Unexpected error on page {page_num}: {e}")
            break  # Exit the outer loop on unexpected errors
        else:
            # This else belongs to the try block. It's executed if the try block completes without an exception
            time.sleep(2)  # Add a delay to avoid rate limiting
            page_num += 1  # Increment the page number
            continue  # Continue to the next page
    
        break  # Break out of the outer loop if any exception occurred
    
    if not all_news:
        st.warning("No results found across all pages.")
        return pd.DataFrame()
...
def get_news_data(method, start_date, end_date, keyword_query):  # Removed max_pages
    headers = {
        "User-Agent":
    ...
    # encoded_query = urllib.parse.quote(full_query) # REMOVE URL ENCODING
    
    base_url = f"https://www.google.com/search?q={full_query}&gl=id&hl=id&lr=lang_id&tbm=nws&num=10"
    
    # 🚀 First scrape:
    if method == "BeautifulSoup":
        news_df = scrape_with_bs4(base_url, headers)
    if method == "Oxylabs":
        news_df = scrape_google_with_oxylabs(full_query, "Indonesia")  # Use full_query
    else:
